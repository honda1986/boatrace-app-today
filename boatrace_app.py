...
def predict_scenario(scores: dict, before_info: dict, detail: dict) -> dict:
    ...
    # 三連単買い目生成 (1着=1号艇固定, 1-2-3/1-3-2は除外)
    bets = []
...
if run:
    ...
    st.session_state.hit_list = hit_list

    if not hit_list:
        st.warning("条件に合致するレースはありませんでした。")

# ── 抽出結果の表示 ──
hit_list = st.session_state.hit_list
...
if hit_list:
    ...
    st.markdown("---")
    st.markdown("**レースをタップしてスコアリング開始 👇**")

    # ── レース選択ボタン ──
    for i, race in enumerate(hit_list):
        race_key = f"{race['venue']}_{race['race_no']}"
        label = f"🚤 {race['race_time']}  【{race['venue']}】 {race['race_no']}R  (1号艇 {race['win_rates'][1]:.2f} / 2号艇 {race['win_rates'][2]:.2f})"
        if st.button(label, key=f"btn_{race_key}", use_container_width=True):
            st.session_state.selected_race_key = race_key

    # ── 選択されたレースのスコアリング＆予想 ──
    if st.session_state.selected_race_key:
        race = None
        for r in hit_list:
            if f"{r['venue']}_{r['race_no']}" == st.session_state.selected_race_key:
                race = r
                break
